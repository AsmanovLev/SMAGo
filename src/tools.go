...
// editDiffSummary returns a short confirmation of what changed.
func editDiffSummary(path, action string, startLine int, oldLines, newLines []string) string {
	nOld := len(oldLines)
	nNew := len(newLines)
	delta := nNew - nOld
	if delta == 0 {
		return fmt.Sprintf("ok: %s on %s line %d (%d lines)", action, path, startLine, nNew)
	}
	sign := "+"
	if delta < 0 {
		sign = ""
	}
	return fmt.Sprintf("ok: %s on %s lines %d-%d (%d->%d lines, %s%d)", action, path, startLine, nOld, nNew, sign, delta)
}
